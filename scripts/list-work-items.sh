...
# Prints "DOMAIN<TAB><owner>/<repo>", then one "<repo><TAB><number><TAB><title><TAB><stage><TAB><url>"
...
bash scripts/list-domains.sh | while IFS=$'\t' read -r REPO _DESC; do
  ...
  gh issue list --repo "$OWNER/$REPO" --state open --limit 1000 \
    ...
    --jq '.[] | [.number, .title, (.labels[].name | select(startswith("stage:"))), .url] | @tsv' |
  while IFS=$'\t' read -r NUMBER TITLE LABEL URL; do
    [ -z "$LABEL" ] && continue
    STAGE="$(stage_name "$LABEL")"
    [ -z "$STAGE" ] && continue
    printf '%s\t%s\t%s\t%s\t%s\n' "$REPO" "$NUMBER" "$TITLE" "$STAGE" "$URL"
  done
done
exit 0
