#!/usr/bin/env bash
# Goal completeness check (R5-R7), run before the requirements agent starts. An
# Issue can reach stage:requirements with an incomplete body (edited by hand, or
# created outside create-work-item.sh's own check), so the runner checks again.
...
gh issue comment "$NUM" --repo "$GITHUB_REPOSITORY" --body \
  "🛑 Goal is incomplete: missing $WHAT. Edit the Issue body to add it, then restart it (dashboard Answer button, or remove and re-add \`stage:requirements\`)."
